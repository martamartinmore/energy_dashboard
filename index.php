<?php
  ...
  $consumo = array();
  for ($i = 1; $i <= 12; $i++) {
    $sql = "SELECT SUM(valor) suma FROM Historicos WHERE MONTH(fecha) = '$i' AND YEAR(fecha) = 2023";
  ...
  $gasto = array();
  for ($i = 1; $i <= 12; $i++) {
    $sql = "SELECT SUM(gasto) suma FROM Historicos WHERE MONTH(fecha) = '$i' AND YEAR(fecha) = 2023";
    $result = $conn->query($sql);  
  ...
  // ---- POTENCIA CONTRATADA ----
  $sql = "SELECT potencia FROM Informacion";
  $result = $conn->query($sql);

  $potencia = array();
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      array_push($potencia, $row["potencia"]);
    }
  } else {
    array_push($potencia, 0);
  }

  // ---- NUMERO DE DISPOSITIVOS CONECTADOS ----
  $sql = "SELECT numero_dispositivos FROM Informacion";
  $result = $conn->query($sql);

  $dispositivos = array();
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      array_push($dispositivos, $row["numero_dispositivos"]);
    }
  } else {
    array_push($dispositivos, 0);
  }

  // ---- CONSUMO TOTAL POR SECTOR ----
  $sql = "SELECT SUM(valor) suma FROM Historicos WHERE MONTH(fecha) = '$i' AND YEAR(fecha) = 2023 AND tipo = 'oficina'";
  $result = $conn->query($sql); 
  
  $consumo_oficina = array();
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      array_push($consumo_oficina, $row["suma"]);
    }
  } else {
    array_push($consumo_oficina, 0);
  }

  $sql = "SELECT SUM(valor) suma FROM Historicos WHERE MONTH(fecha) = '$i' AND YEAR(fecha) = 2023 AND tipo = 'alimentacion'";
  $result = $conn->query($sql); 
  
  $consumo_alimentacion = array();  
  if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
      array_push($consumo_alimentacion, $row["suma"]);
    }
  } else {
    array_push($consumo_alimentacion, 0);
  }
    ...
    <svg style="display:none;">
      ...
      <br>
        <section class="grid1">  
          ...
          <article><div class="flexWrapper"><canvas id="doughnut-chart2" width="200" height="150"></canvas></div></article> 
          <script type="text/javascript">
            const potencia_contratada = <?php echo json_encode($potencia) ?>;
            const potencia_contratada_n = parseInt(potencia_contratada[0]);
            const potencia_array = 10 - potencia_contratada_n;

            const potencia = [potencia_array,potencia_contratada_n];
            ...
            data: {
              ...
              options: {
                ...
                plugins: {
                  ...
                  subtitle: {
                    display: true,
                    text: po.toFixed(2) + ' kw'
                  }
                }
              }
            });
          </script> 
          <article><div class="flexWrapper"><canvas id="doughnut-chart3" width="200" height="150"></canvas></div></article> 
          <script type="text/javascript">
            const numero_dispositivos = <?php echo json_encode($dispositivos) ?>;
            const numero_dispositivos_n = parseInt(numero_dispositivos[0]);
            const dispositivos_array = 200 - numero_dispositivos_n;

            const dispositivos = [dispositivos_array,numero_dispositivos_n];
            ...
            data: {
              ...
              options: {
                ...
                plugins: {
                  title: {
                    display: true,
                    text: 'Nº DISPOSITIVOS CONECTADOS',
                    padding: {
                    top: 10,
                    bottom: 10
                    }
        ...
        <br>
        <section class="grid2">
        <article><div class="flexWrapper"><canvas id="doughnut-chart" width="350" height="350"></canvas></div>
</article> 
          <script type="text/javascript">
            const alimentacion = <?php echo json_encode($consumo_alimentacion) ?>;
            const oficina = <?php echo json_encode($consumo_oficina) ?>;
            const suma_ali = alimentacion.reduce(function(a, b){
              const a1 = parseInt(a);
              const b1 = parseInt(b);
              return a1 + b1;
            });
            const suma_ofi = oficina.reduce(function(a, b){
              const a1 = parseInt(a);
              const b1 = parseInt(b);
              return a1 + b1;
            });
            const sector_data = [suma_ali,suma_ofi]

            new Chart(document.getElementById("doughnut-chart"), {
            type: 'doughnut',
            data: {
              labels: ["Oficina", "Alimentacion"],
              datasets: [
                {                  
              ...
              options: {
                plugins: {
                  title: {
                    display: true,
                    text: 'CONSUMO TOTAL POR SECTOR (kwh)'
                  }
                }
              }
            });
          </script>
        <article><canvas id="bar-chart" width="1000" height="450"></canvas>
</article> 
        <script type="text/javascript">
          const consumo_total2 = <?php echo json_encode($consumo) ?>;
          new Chart(document.getElementById("bar-chart"), {
              ...
              options: {
                ...
                plugins: {
                  ...
                  title: {
                    display: true,
                    text: 'CONSUMO ENERGÉTICO EN 2023 (kwh)'
                  }
                }
              }
          });
